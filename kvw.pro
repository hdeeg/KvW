...
;keywords
                                ...
                                ;notimeoff  disables internal offsetting of time values. Is save to use if eclipse min-time is within +-0.1 of time value
...
;overplot the fitted parabole
  if  ~keyword_set(noplot) then begin
     ...
     for j=0,nfitp-1 do begin
        ...
        Sfit[i]=cp[0]+cp[1]*idfit[j]+cp[2]*idfit[j]^2
        i+=1
     endfor
